<article class="panel panel-default">
    ...
    <div class="panel-body">
        ...
        <div class="media-show-panel" data-media-id="<?= $data->id ?>" style="padding-left: 18%;">
            <a href="#" class="media-show">Показать полностью...</a>
        </div>
        <script type="text/javascript">
            setTimeout(function () {
                if ($("div[data-media-id=<?= $data->id ?>]").height() < 200) {
                    $(".media-show-panel[data-media-id=<?= $data->id ?>]").remove();
                    $("div[data-media-id=<?= $data->id ?>]").css('overflow', 'visible').css('max-height: 100%');
                }
            }, 500);
        </script>
        <?php endif;?>
